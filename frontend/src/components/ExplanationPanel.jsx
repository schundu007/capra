      ...
      <div className="flex-1 overflow-auto scrollbar-thin p-4 space-y-4">
        ...
        {pitch && (
          <div className="animate-fade-in">
            <div className="bg-neutral-800 border border-neutral-700 rounded-md p-4 border-l-4 border-l-white">
              <div className="flex items-center gap-2 mb-3">
                <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                </svg>
                <span className="text-xs font-medium text-neutral-300 uppercase tracking-wide">Solution Pitch (1-2 min)</span>
              </div>
              <p className="text-sm text-neutral-200 whitespace-pre-wrap leading-relaxed">{pitch}</p>
            </div>
          </div>
        )}
        ...
        {explanations && explanations.length > 0 && (
          <>
            <div className="flex items-center gap-2 pt-2">
              ...
              <span className="text-xs font-medium text-neutral-500 uppercase tracking-wide">Line-by-Line</span>
              <div className="h-px flex-1 bg-neutral-700" />
            </div>
              ...
              {explanations.map((item, index) => {
                ...
                return (
                  <div
                    key={index}
                    className={
                      'bg-neutral-800 border border-neutral-700 rounded-md p-3 transition-all duration-200 animate-fade-in ' +
                      (isHighlighted
                        ? 'border-white bg-neutral-750 scale-[1.01]'
                        : '')
                    }
