    ...
    <div className="min-h-screen text-gray-900 overflow-hidden landing-root" style={{ background: 'linear-gradient(180deg, #fdf2f8 0%, #ede9fe 50%, #e0e7ff 100%)', paddingTop: '64px', paddingBottom: '52px' }}>
      ...
      <section className="flex flex-col items-center justify-center text-center px-6 pt-12 pb-12 md:pt-16 md:pb-16">
        ...
        <div className={`mt-5 flex flex-col sm:flex-row items-center gap-3 transition-all duration-700 delay-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
          ...
          {!user && !loading && (
            <button onClick={() => signIn('google')} className="px-8 py-3 bg-white text-gray-700 font-semibold text-sm rounded border border-gray-200 hover:bg-gray-50 transition-colors shadow-sm landing-body flex items-center gap-2">
          ...
          {user && (
            <a href="/app/coding" className="px-8 py-3 bg-white text-gray-700 font-semibold text-sm rounded border border-gray-200 hover:bg-gray-50 transition-colors shadow-sm landing-body">
              Go to App
            </a>
          )}
        </div>
        {!user && !loading && (
          <div className={`mt-3 flex items-center gap-3 transition-all duration-700 delay-400 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
            <button onClick={() => signIn('github')} className="text-sm text-gray-400 hover:text-gray-600 transition-colors landing-body flex items-center gap-1.5">
            ...
            <button onClick={() => signIn('linkedin')} className="text-sm text-gray-400 hover:text-gray-600 transition-colors landing-body flex items-center gap-1.5">
